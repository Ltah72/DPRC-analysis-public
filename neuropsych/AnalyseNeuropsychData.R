...
your_data_file <- read.csv("longitudinal_your_data_file_lined_up_valid_participants.csv")

#for functional-diffusion connectome, longitudinal study (n = 119)
#your_data_file <- read.csv("fMRI_dMRI_longitudinal_your_data_file_lined_up_valid_participants.csv")
...
#posthoc for MANOVA
#need to make separate vectors for this
proc_speed_tests_combined <- cbind(your_data_file$TrailsA.Z, your_data_file$ColorNaming.Z, your_data_file$WordReading.Z, your_data_file$HayBTime1.z) 
classification_group_var <- your_data_file$Group
df_lda_proc_speed <- na.omit(data.frame(your_data_file$Group,your_data_file$TrailsA.Z, your_data_file$ColorNaming.Z, your_data_file$WordReading.Z, your_data_file$HayBTime1.z)) 
#posthoc_manova_proc_speed_z_mod <- lda(df_lda_proc_speed$Group  ~ proc_speed_tests_combined, CV=F)
posthoc_manova_proc_speed_z_mod <- lda(classification_group_var ~ proc_speed_tests_combined, CV=F)
posthoc_manova_proc_speed_z_mod 
#to evaluate whether there are sig. difference between groups, you must visualise it on a plot (no value for this)
